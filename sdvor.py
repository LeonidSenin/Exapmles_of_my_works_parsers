...
def get_links(articul, product, city, city_code):
    df = pd.DataFrame(columns=['indicator', 'subindicator_1', 'region', 'data_source', 'measure_unit', 'periodity',
                               'indicator_value'])
    ...
    if site.status_code == 200:
        ...
        if soup.find('div', {'class': 'product-code'}) != None:
            if str(articul) == soup.find('div', {'class': 'product-code'}).text.split(':')[-1].strip():
                ...
                df_row = pd.DataFrame(
                    {'indicator': ['Цена на ' + product], 'subindicator_1': [str(articul)], 'region': [city],
                     'data_source': 'Сайт ООО Строительный двор',
...
if __name__ == '__main__':
    df_to_db = pd.DataFrame(
        columns=['indicator', 'subindicator_1', 'region', 'data_source', 'measure_unit', 'periodity',
                 'indicator_value'])
